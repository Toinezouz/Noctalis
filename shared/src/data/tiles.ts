 ...
 * ---------------------------------------------------------------------------
 * Le numero d'une etoile determine entierement sa constellation et ses eclats.
 * Toute l'application (plateau, etoiles, carte du ciel, moteur, serveur)
 * lit cette donnee : il est impossible qu'une etoile et sa case de fiche
 * divergent.
 *
 * Constellations (lignes de la fiche officielle, cycle de 5) :
...
/** Ordre cyclique des constellations, tel que sur la fiche officielle. */
...
/** Nombre total de etoiles du jeu. */
export const TILE_COUNT = 60;

/** Nombre de colonnes de la carte du ciel (60 / 5 constellations). */
export const SHEET_COLUMNS = TILE_COUNT / COLOR_ORDER.length;

/** Nombre de etoiles secretes par joueur (une par constellation). */
export const SECRET_TILE_COUNT = COLOR_ORDER.length;

/** Libelles francais des constellations (accessibilite, aria-labels, textes). */
...
/** Les 60 etoiles, indexees de 1 a 60 (index 0 inclus mais inutilise). */
...
/** Recupere une etoile par son numero. Leve une erreur si le numero est invalide. */
...
/** Les 12 numeros d'une constellation donnee, par ordre croissant. */
export function tilesOfColor(color: TileColor): readonly Tile[] {
  return TILES.filter((t) => t.color === color);
}

/**
 * Grille de la carte du ciel : 5 lignes (constellations) x 12 colonnes.
 * `SHEET_GRID[i][j]` est l'etoile de la constellation `COLOR_ORDER[i]`, colonne `j+1`.
 */
export const SHEET_GRID: readonly (readonly Tile[])[] = Object.freeze(
  COLOR_ORDER.map((color) => Object.freeze(tilesOfColor(color))),
);

/** `true` si `n` est un numero de etoile valide. */
